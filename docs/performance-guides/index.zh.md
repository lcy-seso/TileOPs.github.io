...
| [优化 access pattern](memory-bound-kernels.md) | 访存合并、用 pad 消除 bank conflict、整行读进 fragment、让输入与输出都用满向量访问 |
| [超越函数的开销与精度](transcendental.md) | 在 fp32 上做运算、用恒等式减少超越函数的个数 |
| [TileLang 使用陷阱](tilelang-pitfalls.md) | 宏按提及展开、简化器消掉 `x + 0.0`、简化器把 `x != x` 判 NaN 折成恒假 |
