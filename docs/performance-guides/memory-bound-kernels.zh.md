...
一个 warp 的 32 个线程同步执行这个循环，同一次迭代里 `c` 对它们取同一个值、`tx` 取 0 到 31：这时 32 个线程以等间隔访问 shared memory。例如 `chunk = 64` 时，这 32 个线程在同一次迭代里读的是第 `c`、第 `64 + c`、第 `128 + c`、…… 第 `1984 + c` 个元素 —— 相邻两个线程相差 `chunk` 个元素。这个差值称为这段访问的 **stride**，**bank 冲突只取决于它**，与数组声明成几维、下标怎么写都无关。

设元素为 `E` 字节，stride 换算成 word 数是 `stride_words = chunk * E / 4` 个，当：

1. **`stride_words` 是整数。**
2. **32 个线程读的是 32 个不同的 word。** 落在同一个 word 上的线程走广播，不产生冲突。

32 个线程落到多少条不同的 bank 上，由 `stride_words` 与 32 的最大公约数决定：
...
上面的式子假定每个线程一次只读一个 word。而我们通常会以向量化的方式一次访问 $w$ 个连续的 word，$w$ 取 1、2、4，对应 32 bit、`float2` 的 8 字节、`float4` 的 16 字节。$w$ 变了，pad 的要求也随之改变。**完整的判据如下**，两条都要满足：

| 消费侧的访问位宽 | $w$ | 冲突路数要求 | 段起点的对齐要求 | fp32、`chunk = 64` 上最快的 pad |
| --- | --- | --- | --- | --- |
...
两条要求方向相反，所以不能只看一条：**位宽越宽，对冲突路数的要求越松，对对齐的要求越紧**。奇数 pad 对标量读是最优解，对向量化读却是最差的 —— 它让段起点错开 4 字节，宽指令发不出来。

#### 冲突路数为什么随位宽放宽

整个 warp 要取 $32w$ 个 word，而 shared memory 每周期最多处理 32 个 word（32 条 bank 各一个），所以这条指令至少要 $w$ 个周期 —— 这是位宽带来的下界，与地址无关，冲突指的是超出这个下界的部分。线程 $t$ 取的是第 $St$ 到 $St + w - 1$ 个 word。$St \bmod 32$ 只取到 $g = \gcd(S, 32)$ 的倍数，也就是 $32/g$ 条 bank，每条被碰到 $g$ 次；再叠加 $j = 0, \dots, w-1$ 的平移。$g$ 与 $w$ 都是 2 的幂，必有一个整除另一个，于是分两种情形：
...
$w = 1$ 时它退化成上面那个 $\gcd$。推导的最后一步假定硬件能把任意一组无冲突的 word 凑进一个周期，而 NVIDIA 没有公开 64 bit 与 128 bit 访问时 lane 的实际分组方式，所以这里只能写成不等式。

实测（H200，fp32，`chunk = 64`，消费循环重复 32 遍使 shared 侧成为瓶颈，耗时随遍数成比例）给出每种组合比同位宽最快的那一行慢多少倍：
...
对角线上的三个 1.00 就是上表那三行推荐：$\gcd$ 每放宽一档，最快的位宽也宽一档。$g \le w$ 那一侧下界是紧的；$g > w$ 那一侧 128 bit 实测恰好是下界的 2 倍（4.04 对 2、7.99 对 4、15.71 对 8），硬件没有达到理想调度。$\gcd = 1$ 那一行的 2.27 是对齐造成的：$S = 65$ word 即 260 字节，不是 16 的倍数。
...
20 个配置里，带宽随预测的路数单调下降 —— 1 路与 2 路在 3.0 以上，4 路降到 2.6 附近，8 路及以上跌到 1.6 以下。在这组条件（512 MB、填满整卡）下，公式可以直接用来选 pad，不必逐个试；block 数不足时冲突路数不再是第一因素，见取舍里「对齐与低冲突路数不可兼得」一段。
...
**对齐与低冲突路数不可兼得，按 kernel 所在的区间取舍。** stride 保持 16 字节对齐意味着 `(chunk + pad) * E` 是 16 的倍数，折成 word 后是 4 的倍数，`gcd(stride_words, 32)` 至少是 4 —— 16 字节对齐的 stride 至少落在 4 路冲突，两者只能保一头。而对齐的影响不止 shared 一侧：staged 的搬运循环里，global 载入的宽度与 shared 存储的宽度绑在一起，对齐丢失时 global 读一并变窄。两个区间的答案相反（见前面的[三个区间](#regime)）：

- **填满整卡时，冲突路数是主要因素。** 上表的条件（512 MB、锁频）下，`pad = 8`（16 字节）的 2.57 到 2.76 明显低于 `pad = 4` 的 3.29 到 3.70，`pad = 16`（32 字节）更落到 8 路甚至 16 路。
- **block 数不足时结论反转，载入宽度更重要。** 一个只有 64 个 block 的按行前缀和（64 × 32768，bf16，512 线程，chunk = 64，H200，时钟未锁，TileOPs benchmark 的 `device_busy` 中位数）上，16 字节对齐的 `pad = 8`（4 路）测得 1.331 TB/s，2 路的 `pad = 4` 只有 1.057，反而慢 21%。
...
**这一条只在数据经过 shared memory 时适用。** 按[第 1 条](#coalescing)的取舍，$V$ 小时用向量化的 blocked，数据直接进寄存器，不经 shared memory，没有 bank 冲突可言；$V$ 大到寄存器压力压低占用率时才改用 staged，以及整行需要被 block 内所有线程共享时，这一条才适用。
