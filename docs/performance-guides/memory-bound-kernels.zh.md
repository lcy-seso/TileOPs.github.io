...
下文每一条给出的排序都在同一组条件下测得：**输入大于 L2 的 60 MiB，且 block 数足以填满整卡**（H200 有 132 个 SM）。两个条件同时成立时，kernel 的耗时由 DRAM 带宽决定，访存模式的差别就是成绩的差别；任一条件不成立时，决定成绩的因素换了，排序不再保证成立，个别结论还会反转。套用条目之前先对照下表判断 kernel 处在哪个区间：

| 区间 | 判据 | 决定成绩的因素 | 条目的用法 |
| --- | --- | --- | --- |
| 带宽饱和 | 输入 > 60 MiB，block 数在 SM 数的两倍以上 | DRAM 带宽，即 sector 利用率 | 排序直接可用 |
| 数据小 | 输入装得进 L2，单次耗时在几十微秒以内 | kernel 发射的固定开销、缓存状态 | 只需避开各条的反例 |
| block 少 | block 数不到 SM 数的两倍 | 每条载入指令的宽度、在飞的字节数 | 保住载入宽度优先，改动逐个实测 |

后两行的依据：

- **数据小时，成绩由发射开销与缓存状态决定，写法分不出名次。** 同一组行求和 kernel——staged、交给 layout inference 的逐元素 `T.Parallel`、`T.copy` 进 fragment、手写向量化 blocked（fp16，256 线程，时钟未锁）——在 65536 × 4096（512 MB）上测得 4.20 到 4.43 TB/s，都在实测带宽的顶附近，差别不超过 6%；换成 2048 × 4096（16 MB，进 L2）后单次耗时十几微秒，同一写法两轮之间可差三倍，写法之间没有稳定名次。这个区间里更换访存模式没有收益，该查的是发射次数与算法本身。
- **block 少时，宽度换来的收益大于合并规则算出的差别。** warp 不够多，靠请求数量掩延迟的路走不通，只能靠每个请求更宽、每个线程压更多在飞字节。凡是「牺牲载入宽度换别的好处」的改法在这个区间都可能反转；[第 2 条](#bank-conflict)的 pad 取舍给出一个实测的反例。
...
**对齐与低冲突路数不可兼得，按 kernel 所在的区间取舍。** stride 保持 16 字节对齐意味着 `(chunk + pad) * E` 是 16 的倍数，折成 word 后是 4 的倍数，`gcd(stride_words, 32)` 至少是 4——16 字节对齐的 stride 至少落在 4 路冲突，两者只能保一头。而对齐的影响不止 shared 一侧：staged 路线的搬运循环里，global 载入的宽度与 shared 存储的宽度绑在一起，对齐丢失时 global 读一并变窄。两个区间的答案相反（[适用范围](#regime)）：

- **填满整卡时，冲突路数说了算。** 上表的条件（512 MB、锁频）下，`pad = 8`（16 字节）的 2.57 到 2.76 明显低于 `pad = 4` 的 3.29 到 3.70，`pad = 16`（32 字节）更落到 8 路甚至 16 路。
- **block 数不足时反转，载入宽度说了算。** 一个只有 64 个 block 的按行前缀和（64 × 32768，bf16，512 线程，chunk = 64，H200，时钟未锁，TileOPs benchmark 的 `device_busy` 中位数）上，16 字节对齐的 `pad = 8`（4 路）测得 1.331 TB/s，2 路的 `pad = 4` 只有 1.057，反而慢 21%。
