...
**对齐与低冲突路数不可兼得，按 kernel 所在的区间取舍。** stride 保持 16 字节对齐意味着 `(chunk + pad) * E` 是 16 的倍数，折成 word 后是 4 的倍数，`gcd(stride_words, 32)` 至少是 4——16 字节对齐的 stride 至少落在 4 路冲突，两者只能保一头。而对齐的影响不止 shared 一侧：staged 路线的搬运循环里，global 载入的宽度与 shared 存储的宽度绑在一起，对齐丢失时 global 读一并变窄。两个区间的答案相反（见前面的[三个区间](#regime)）：
